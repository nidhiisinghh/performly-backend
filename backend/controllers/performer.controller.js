...
class PerformerController {
  ...
  static async getAllPerformers(req, res) {
    try {
      const performers = await Performer.find().populate(
        "userId",
        "name profilePic",
      );
  ...
  static async getPerformersByCategory(req, res) {
    try {
      ...
      const performers = await Performer.find({ category }).populate(
        "userId",
      ...
      res
        .status(500)
        .json({ message: "Error fetching performers", error: error.message });
    }
  }
  ...
  static async getPerformerById(req, res) {
    try {
      ...
      res
        .status(500)
        .json({ message: "Error fetching performer", error: error.message });
    }
  }
  ...
  static async createProfile(req, res) {
    try {
      ...
      if (user.type !== 1) {
        return res
          .status(403)
          .json({ message: "Only performers can create performer profiles" });
      }
      ...
      if (existingProfile) {
        return res
          .status(400)
          .json({ message: "Performer profile already exists" });
      }
      ...
      if (
        ...
        return res.status(400).json({
          message:
            "Category, subCategory, experience, and pricing are required",
        });
      }
      ...
      await profile.save();
      await profile.populate("userId", "name email phone profilePic");
