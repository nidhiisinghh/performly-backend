...
const getClientBookings = async (req, res) => {
  try {
    const bookings = await Booking.find({ clientId: req.params.id }).sort({
      createdAt: -1,
    });
    ...
    const bookingsWithName = await Promise.all(
...
const detailsByID = async (req, res) => {
  try {
    ...
    const performerDetails = await Performer.findOne({
      userId: bookingDetails.performerId,
